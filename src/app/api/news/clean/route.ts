        ...
        try {
          await Promise.allSettled([
            prisma.news.update({
              ...
              console.error('Error updating news record during cleanup:', post.id, error);
              throw error;
            }),
            post.telegramChatId &&
              ...
              bot.editMessageReplyMarkup(
                ...
                console.error('Error editing Telegram message during cleanup:', post.id, error);
                // Don't throw here as telegram errors shouldn't block the cleanup
              }),
          ]);
        } catch (error) {
          console.error('Error processing news cleanup for post:', post.id, error);
          continue;
        }
      }
    }
